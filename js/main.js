...
function onRadioButtonChange() {
  ...
  let onlineText = document.getElementById('online_text');
  let venueText = document.getElementById('venue_text');

  const onlineButton = document.form1.online.checked;
  // const venueButton = document.form1.venue.checked;

  if (onlineButton === true) {
...
// HTMLのinputタグ内での onchange で実行できるようにしている。
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (validationForm) {
    ...
    //equal-to クラスを指定された要素の集まり
    const reserveElems = document.querySelectorAll('.reserve');
    ...
    // });
      ...
      equalToElems.forEach((elem) => {
        ...
        if (elem.value !== '' && equalToElem.value !== '') {
          if (equalToElem.value !== elem.value) {
            createError(elem, 'メールアドレスが一致していません');
            e.preventDefault();
          }
        }
      });
      ...
      //     //test() メソッドで値を判定し、マッチしなければエラーを表示してフォームの送信を中止
      //     if (!pattern.test(elem.value)) {
      //       createError(elem, '予約日の形式が正しくないようです。');
      //       e.preventDefault();
      //     }
      //   }
      // });
